...
var teamArr = [];
var getTeamObj = function(index){
	if(teamArr[$("#teamId"+index).text().trim()]){
		doUpdate(index,teamObj);
	}else{
		var q = new Bmob.Query(Bmob.Object.extend('team'));
		q.get($("#teamId"+index).text().trim(),{
			success:function(res){
				teamArr[res.id] = res;
				doUpdate(index,res);
			},
...
T;
        ...
        for ($row = 2; $row <= $highestRow; $row ++) { // 行数是以第1行开始
			...
			$teamId = $sheet->getCell('M' . $row)->getValue();
			...
			echo "<td id='teamId".$index."'>" . $teamId . "</td>";
            echo "</tr>";
            $index++;
        }
        echo "</table>";
